...
/************************************************************************
*  Program Name:		SteeringBehaviors.h								*
*  Name:				James B. Strickler II							*
*  Date:				January 25, 2013								*
*  Description:			Header for the SteeringBehavior class			*
*						to encapsulatesteering behaviors				*
*						for an Entity									*
*  Update:																*
*																		*
************************************************************************/
...
private:
  ...
  Entity*     m_pTargetAgent1;
  Entity*     m_pTargetAgent2;
  ...
  //the distance (squared) a Entity has to be from a path waypoint before
  ...
  //Arrive makes use of these to determine how quickly a Entity
  ...
  Vector2D Pursuit(const Entity* agent);

  //this behavior maintains a position, in the direction of offset
  //from the target Entity
  Vector2D OffsetPursuit(const Entity* agent, const Vector2D offset);

  //this behavior attempts to evade a pursuer
  Vector2D Evade(const Entity* agent);
  ...
  //this results in a steering force that attempts to steer the Entity
  //to the center of the vector connecting two moving agents.
  Vector2D Interpose(const Entity* EntityA, const Entity* EntityB);

  //given another agent position to hide from and a list of BaseGameEntitys this
  //method attempts to put an obstacle between itself and its opponent
  Vector2D Hide(const Entity* hunter, const std::vector<BaseGameEntity*>& obstacles);


  // -- Group Behaviors -- //

  Vector2D Cohesion(const std::vector<Entity*> &agents);
  
  Vector2D Separation(const std::vector<Entity*> &agents);

  Vector2D Alignment(const std::vector<Entity*> &agents);

  //the following three are the same as above but they use cell-space
  //partitioning to find the neighbors
  Vector2D CohesionPlus(const std::vector<Entity*> &agents);
  Vector2D SeparationPlus(const std::vector<Entity*> &agents);
  Vector2D AlignmentPlus(const std::vector<Entity*> &agents);
...
public:
  ...
  void      SetTargetAgent1(Entity* Agent){m_pTargetAgent1 = Agent;}
  void      SetTargetAgent2(Entity* Agent){m_pTargetAgent2 = Agent;}
  ...
  void PursuitOn(Entity* v){m_iFlags |= pursuit; m_pTargetAgent1 = v;}
  void EvadeOn(Entity* v){m_iFlags |= evade; m_pTargetAgent1 = v;}
  ...
  void InterposeOn(Entity* v1, Entity* v2){m_iFlags |= interpose; m_pTargetAgent1 = v1; m_pTargetAgent2 = v2;}
  void HideOn(Entity* v){m_iFlags |= hide; m_pTargetAgent1 = v;}
  void OffsetPursuitOn(Entity* v1, const Vector2D offset){m_iFlags |= offset_pursuit; m_vOffset = offset; m_pTargetAgent1 = v1;}  
