...
static PhysicsEngine::Random random;

/**
 * Fireworks are particles, with additional data for rendering and
 * evolution.
 */
class Firework : public PhysicsEngine::Particle
{
public:
...
struct FireworkRule
{
    ...
    PhysicsEngine::Vector3D minVelocity;

    /** The maximum relative velocity of this firework. */
    PhysicsEngine::Vector3D maxVelocity;
    ...
    void setParameters(unsigned type, double minAge, double maxAge,
        const PhysicsEngine::Vector3D &minVelocity, const PhysicsEngine::Vector3D &maxVelocity,
        double damping)
    {
    ...
    {
        firework->type = type;
        firework->age = random.randomDouble(minAge, maxAge);

        PhysicsEngine::Vector3D vel;
        if (parent) {
        ...
        else
        {
            ...
            int x = (int)random.randomDoubleInt(3) - 1;
        ...
        firework->setAcceleration(PhysicsEngine::Vector3D::GRAVITY);
...
void FireworksDemo::initFireworkRules()
{
    ...
    rules[0].setParameters(
        1, // type
        0.5f, 1.4f, // age range
        PhysicsEngine::Vector3D(-5, 25, -5), // min velocity
        PhysicsEngine::Vector3D(5, 28, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[1].setParameters(
        2, // type
        0.5f, 1.0f, // age range
        PhysicsEngine::Vector3D(-5, 10, -5), // min velocity
        PhysicsEngine::Vector3D(5, 20, 5), // max velocity
        0.8 // damping
        );
    ...
    rules[2].setParameters(
        3, // type
        0.5f, 1.5f, // age range
        PhysicsEngine::Vector3D(-5, -5, -5), // min velocity
        PhysicsEngine::Vector3D(5, 5, 5), // max velocity
        0.1 // damping
        );
    ...
    rules[3].setParameters(
        ...
        PhysicsEngine::Vector3D(-20, 5, -5), // min velocity
        PhysicsEngine::Vector3D(20, 5, 5), // max velocity
        0.2 // damping
        );
    ...
    rules[4].setParameters(
        5, // type
        0.5f, 1.0f, // age range
        PhysicsEngine::Vector3D(-20, 2, -5), // min velocity
        PhysicsEngine::Vector3D(20, 18, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[5].setParameters(
        ...
        3, 5, // age range
        PhysicsEngine::Vector3D(-5, 5, -5), // min velocity
        PhysicsEngine::Vector3D(5, 10, 5), // max velocity
        0.95 // damping
        );
    ...
    rules[6].setParameters(
        7, // type
        4, 5, // age range
        PhysicsEngine::Vector3D(-5, 50, -5), // min velocity
        PhysicsEngine::Vector3D(5, 60, 5), // max velocity
        0.01 // damping
        );
    ...
    rules[7].setParameters(
        8, // type
        0.25f, 0.5f, // age range
        PhysicsEngine::Vector3D(-1, -1, -1), // min velocity
        PhysicsEngine::Vector3D(1, 1, 1), // max velocity
        0.01 // damping
        );
    ...
    rules[8].setParameters(
        9, // type
        3, 5, // age range
        PhysicsEngine::Vector3D(-15, 10, -5), // min velocity
        PhysicsEngine::Vector3D(15, 15, 5), // max velocity
        0.95 // damping
        );
...
{
    return "PhysicsEngine><RABBID>< Fireworks Demo";
}
...
void FireworksDemo::display()
{
        ...
        firework++)
    {
        ...
        if (firework->type > 0)
        {
            ...
            PhysicsEngine::Vector3D &pos = firework->getPosition();
