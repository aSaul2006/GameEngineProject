 ...
 */
class Projectiles : public Application
{
    enum ShotType
    {
        UNUSED = 0,
...
const char* Projectiles::getTitle()
{
    return "PhysicsEngine><RABBID>< Projectiles Demo";
}

void Projectiles::fire()
{
...
void Projectiles::update()
{
    ...
    for (AmmoRound *shot = ammo; shot < ammo+ammoRounds; shot++)
    {
        ...
        {
            // Run the physics
            shot->particle.integrate(dElapsedFrameTime);
...
void Projectiles::mouse(int button, int state, int x, int y)
{
    // Fire the current weapon.
    if (state == GLUT_DOWN) fire();
}

void Projectiles::key(unsigned char key)
{
    switch(key)
    {
