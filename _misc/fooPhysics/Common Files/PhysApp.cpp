...
{
    particleArray = new PhysicsEngine::Particle[particleCount];
...
void MassAggregateApplication::display()
{
    ...
    PhysicsEngine::ParticleWorld::Particles &particles = world.getParticles();
    for (PhysicsEngine::ParticleWorld::Particles::iterator p = particles.begin();
        p != particles.end();
        p++)
    {
        PhysicsEngine::Particle *particle = *p;
        PhysicsEngine::Vector3D &pos = particle->getPosition();
        glPushMatrix();
...
void MassAggregateApplication::update()
{
    ...
    double dElapsedFrameTime = (float)TimingData::get().lastFrameDuration * 0.001f;
...
void RigidBodyApplication::update()
{
    ...
    double dElapsedFrameTime = (float)TimingData::get().lastFrameDuration * 0.001f;
    if (dElapsedFrameTime <= 0.0f) return;
    else if (dElapsedFrameTime > 0.05f) dElapsedFrameTime = 0.05f;
    ...
    // Update the objects
    updateObjects(dElapsedFrameTime);
...
void RigidBodyApplication::drawDebug()
{
    ...
    glBegin(GL_LINES);
    for (unsigned i = 0; i < cData.wNumberOfContacts; i++)
    {
        ...
        PhysicsEngine::Vector3D vec = contacts[i].m_v3PointOfContactWorld;
        glVertex3f(vec.x, vec.y, vec.z);

        vec += contacts[i].m_v3NormalofContactWorld;
        glVertex3f(vec.x, vec.y, vec.z);
    }
