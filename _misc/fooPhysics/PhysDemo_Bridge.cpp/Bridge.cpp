...
*                   object needed to run PhysDemo_Bridge                               *
...
{
    PhysicsEngine::ParticleCableConstraint *supports;
    PhysicsEngine::ParticleCable *cables;
    PhysicsEngine::ParticleRod *rods;

    PhysicsEngine::Vector3D massPos;
    PhysicsEngine::Vector3D massDisplayPos;
...
massPos(0,0,0.5f)
{
    ...
    for (unsigned i = 0; i < 12; i++)
    {
        ...
        particleArray[i].setAcceleration(PhysicsEngine::Vector3D::GRAVITY);
        particleArray[i].clearAccumulator();
    }

    // Add the links
    cables = new PhysicsEngine::ParticleCable[CABLE_COUNT];
    ...
    supports = new PhysicsEngine::ParticleCableConstraint[SUPPORT_COUNT];
    for (unsigned i = 0; i < SUPPORT_COUNT; i++)
    {
        supports[i].particle = particleArray+i;
        supports[i].anchor = PhysicsEngine::Vector3D(
    ...
    rods = new PhysicsEngine::ParticleRod[ROD_COUNT];
...
void BridgeDemo::updateAdditionalMass()
{
    ...
    int x = int(massPos.x);
    double xp = fmod(massPos.x, double(1.0));
    if (x < 0)
    {
        x = 0;
        xp = 0;
    ...
    int z = int(massPos.z);
    double zp = fmod(massPos.z, double(1.0));
    if (z < 0)
    {
        z = 0;
        zp = 0;
...
void BridgeDemo::display()
{
    ...
    {
        PhysicsEngine::Particle **particles = rods[i].particle;
        const PhysicsEngine::Vector3D &p0 = particles[0]->getPosition();
        const PhysicsEngine::Vector3D &p1 = particles[1]->getPosition();
    ...
    {
        PhysicsEngine::Particle **particles = cables[i].particle;
        const PhysicsEngine::Vector3D &p0 = particles[0]->getPosition();
        const PhysicsEngine::Vector3D &p1 = particles[1]->getPosition();
        glVertex3f(p0.x, p0.y, p0.z);
        glVertex3f(p1.x, p1.y, p1.z);
    ...
    for (unsigned i = 0; i < SUPPORT_COUNT; i++)
    {
        const PhysicsEngine::Vector3D &p0 = supports[i].particle->getPosition();
        const PhysicsEngine::Vector3D &p1 = supports[i].anchor;
        glVertex3f(p0.x, p0.y, p0.z);
        glVertex3f(p1.x, p1.y, p1.z);
    }
    glEnd();
...
{
    return "PhysicsEngine><RABBID>< Bridge Demo";
}
